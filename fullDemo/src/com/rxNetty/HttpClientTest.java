...
public class HttpClientTest {
	...
	public static void init() {
		HttpServerBuilder<ByteBuf, ByteBuf> builder = new HttpServerBuilder<ByteBuf, ByteBuf>(new ServerBootstrap().group(new NioEventLoopGroup(10, new RxServerThreadFactory())),
				port, new RequestProcessor());
	...
	public void testConnectionClose() throws Exception {
		...
		RxClient.ClientConfig clientConfig = new Builder(null).readTimeout(timeoutMillis, TimeUnit.MILLISECONDS).build();

		final Observable<ByteBuf> content = client.submit(HttpClientRequest.createGet("test/singleEntity"), connectionObservable)
	...
	public void testChunkedStreaming() throws Exception {
		HttpClient<ByteBuf, ServerSentEvent> client = RxNetty.createHttpClient("localhost", port, PipelineConfigurators.<ByteBuf> clientSseConfigurator());
		Observable<HttpClientResponse<ServerSentEvent>> response = client.submit(HttpClientRequest.createGet("test/stream"));
	...
	public void testWriteWithErrorContent() throws Exception {
		...
		HttpClientRequest<ByteBuf> request = HttpClientRequest.createPost("/").withContentSource(errSource);
	...
	public void testMultipleChunks() throws Exception {
		HttpClient<ByteBuf, ServerSentEvent> client = RxNetty.createHttpClient("localhost", port, PipelineConfigurators.<ByteBuf> clientSseConfigurator());
		Observable<HttpClientResponse<ServerSentEvent>> response = client.submit(HttpClientRequest.createDelete("test/largeStream"));
	...
	public void testMultipleChunksWithTransformation() throws Exception {
		HttpClient<ByteBuf, ServerSentEvent> client = RxNetty.createHttpClient("localhost", port, PipelineConfigurators.<ByteBuf> clientSseConfigurator());
		Observable<HttpClientResponse<ServerSentEvent>> response = client.submit(HttpClientRequest.createGet("test/largeStream"));
	...
	public void testSingleEntity() throws Exception {
		HttpClient<ByteBuf, ByteBuf> client = RxNetty.<ByteBuf, ByteBuf> newHttpClientBuilder("localhost", port).enableWireLogging(LogLevel.ERROR).build();
	...
	public void testPostWithRawContentSource() {
		PipelineConfigurator<HttpClientResponse<ByteBuf>, HttpClientRequest<String>> pipelineConfigurator = PipelineConfigurators.httpClientConfigurator();
	...
	public void testNonChunkingStream() throws Exception {
		HttpClient<ByteBuf, ServerSentEvent> client = RxNetty.createHttpClient("localhost", port, PipelineConfigurators.<ByteBuf> clientSseConfigurator());
		Observable<HttpClientResponse<ServerSentEvent>> response = client.submit(HttpClientRequest.createGet("test/nochunk_stream"));
		final List<String> result = new ArrayList<String>();
	...
	public void testTimeout() throws Exception {
		...
		Observable<HttpClientResponse<ByteBuf>> response = client.submit(HttpClientRequest.createGet("test/timeout?timeout="
				+ timeoutMillis * 2 /* Create bigger wait than timeout */));
	...
	public void testNoReadTimeout() throws Exception {
		RxClient.ClientConfig clientConfig = new Builder(null).readTimeout(2, TimeUnit.SECONDS).build();
		HttpClient<ByteBuf, ByteBuf> client = new HttpClientBuilder<ByteBuf, ByteBuf>("localhost", port).config(clientConfig).build();
		Observable<HttpClientResponse<ByteBuf>> response = client.submit(HttpClientRequest.createGet("test/singleEntity"));
	...
	public void testLargeHeaders() throws Exception {
		HttpClient<ByteBuf, ByteBuf> client = RxNetty.<ByteBuf, ByteBuf> newHttpClientBuilder("localhost", port)
				.pipelineConfigurator(new HttpClientPipelineConfigurator<ByteBuf, ByteBuf>(1024, 10, 1024)).enableWireLogging(LogLevel.ERROR).build();
		Observable<HttpClientResponse<ByteBuf>> response = client.submit(HttpClientRequest.createGet("test/singleEntity"));
		final List<String> result = new ArrayList<String>();
		response.flatMap(new Func1<HttpClientResponse<ByteBuf>, Observable<String>>() {
			@Override
	...
	public void testDisableAutoRelease() throws Exception {
		HttpClient<ByteBuf, ByteBuf> client = RxNetty.<ByteBuf, ByteBuf> newHttpClientBuilder("localhost", port).disableAutoReleaseBuffers().enableWireLogging(LogLevel.ERROR)
				.build();
		Observable<HttpClientResponse<ByteBuf>> response = client.submit(HttpClientRequest.createGet("test/singleEntity"));
	...
	private static void readResponseContent(Observable<HttpClientResponse<ServerSentEvent>> response, final List<String> result) {
		response.flatMap(new Func1<HttpClientResponse<ServerSentEvent>, Observable<ServerSentEvent>>() {
			@Override
